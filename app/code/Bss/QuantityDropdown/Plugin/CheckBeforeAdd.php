...
class CheckBeforeAdd
{
    ...
    public function beforeUpdateItems(
        ...
        foreach ($data as $itemId => $qty) {
            ...
            $validate = $this->validateQty($product, $product->getName(), $qty['qty'], $item->getQty());
            if ($validate != '') {
                throw new \Magento\Framework\Exception\LocalizedException(__($validate));
            }
        }
     ...
     * @SuppressWarnings(PHPMD.CyclomaticComplexity)
     */
    protected function validateQty($product, $productName, $qty, $oldQty = -1)
    {
        $message = '';
        ...
        if ($enableQtyOption != 1 || $type != 'none') {
            ...
            if (in_array($type, $checkTypeCustom)) {
                ...
                if ($oldQty == -1) {
                    $addedQty = (int)$this->request->getParam('qty', 1);
                } else {
                    $addedQty = $qty - $oldQty;
                }
                $addedQty = abs($addedQty);
                if ($addedQty > 0 && !empty($cus) && /*!in_array($qty, $cus) &&*/ !in_array($addedQty, $cus)) {
                    $multipleItems = array_filter($cus, function($item) use(&$addedQty) {
                        return $addedQty % $item === 0;
                    });
                    if (count($multipleItems) == 0) {
                        $custom = implode(',', $cus);
