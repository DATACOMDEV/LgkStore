...
$idAttributo = 44;  //description

//Recupero le categorie che hanno già un valore sullo store in lingua per l'attributo interessato
$idProdottiValorizzati = [];
$results = $conn->query(sprintf('SELECT entity_id FROM catalog_category_entity_text
...
$textData = [];
$results = $conn->query(sprintf('SELECT entity_id, value FROM catalog_category_entity_text
...
foreach ($textData as $entityId => $value) {
    ...
    foreach ($deeplData['translations'] as $index => $data) {
        ...
        $oldValue = $conn->query(sprintf('SELECT value FROM catalog_category_entity_text
