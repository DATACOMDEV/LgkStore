...
$idAttributo = 72;  //description
//$idAttributo = 73;  //short_description

//Recupero gli articoli che hanno già un valore sullo store francese per l'attributo interessato
$idProdottiValorizzati = [];
$results = $conn->query(sprintf('SELECT entity_id FROM catalog_product_entity_text
...
$textData = [];
$results = $conn->query(sprintf('SELECT entity_id, value FROM catalog_product_entity_text
...
foreach ($textData as $entityId => $value) {
    ...
    if (!is_null($glossaryToUse) && !empty($glossaryToUse)) {
        $postFields[] = "glossary_id=".$glossaryToUse;
    }
    ...
    foreach ($deeplData['translations'] as $index => $data) {
        ...
        $oldValue = $conn->query(sprintf('SELECT value FROM catalog_product_entity_text
        WHERE attribute_id=%d AND store_id=%d AND entity_id=%d', $idAttributo, $idTargetStore, $entityId));
        ...
        $notTranslated = false;
        while ($row = $oldValue->fetch_assoc()) {
