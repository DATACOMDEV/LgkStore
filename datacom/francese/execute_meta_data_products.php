...
WHERE attribute_id=70 AND store_id=%d', $idStoreFrancese));
if (!$results) {
...
foreach ($idAttributVarchar as $idAttributo) {
    foreach ($idProdottiValorizzati as $entityId => $newValue) {
        ...
        $stmnt->bind_param('iiiss', $idAttributo, $idStoreFrancese, $entityId, $newValue, $newValue);
        $stmnt->execute();
...
foreach ($idAttributiText as $idAttributo) {
    foreach ($idProdottiValorizzati as $entityId => $newValue) {
        $stmnt = $conn->prepare('INSERT INTO catalog_product_entity_text (attribute_id, store_id, entity_id, value) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE value=?');
        $stmnt->bind_param('iiiss', $idAttributo, $idStoreFrancese, $entityId, $newValue, $newValue);
        $stmnt->execute();
